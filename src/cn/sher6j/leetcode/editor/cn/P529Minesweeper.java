...
class Solution {
    ...
    public void dfs(char[][] board, int x, int y) {
        int count = countSweepers(board, x, y);
        ...
        if (count > 0) {
            ...
            board[x][y] = 'B';
            for (int i = 0; i < 8; i++) {
                int xChange = x + dirX[i], yChange = y + dirY[i];
                if (xChange < 0 || xChange >= board.length ||
                    yChange < 0 || yChange >= board[0].length ||
                    board[xChange][yChange] != 'E') {
                    continue;
                }
                dfs(board, xChange, yChange);
            }
        }
    }
    
    public int countSweepers(char[][] board, int x, int y) {
        int count = 0; // (x, y)周围的雷数
        for (int i = 0; i < 8; i++) {
            int xChange = x + dirX[i], yChange = y + dirY[i];
            if (xChange < 0 || xChange >= board.length ||
                    yChange < 0 || yChange >= board[0].length) {
            ...
            if (board[xChange][yChange] == 'M') count++;
        }
        return count;
    }
}
